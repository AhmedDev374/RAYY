...
export default function DiseaseMapPage() {
  ...
  return (
    <div dir="rtl" lang="ar" className="font-arabic">
      <h1 className="text-2xl font-bold text-leaf-800 mb-2">خريطة الأمراض</h1>
      <p className="text-sm text-gray-600 mb-4">تقارير مجهولة الهوية يشاركها المجتمع بموافقته.</p>
            ...
            return (
              <CircleMarker key={i} center={[lat, lng]} radius={6 + p.count * 2} pathOptions={{ color: '#16a34a' }}>
                <Popup>
                  ...
                  <br />
                  عدد البلاغات: {p.count}
                  {p.region && <><br />المنطقة: {p.region}</>}
                </Popup>
              </CircleMarker>
            );
          })}
      ...
      <ul className="text-sm space-y-1">
        {points.map((p, i) => (
          <li key={i}>{p.disease}: {p.count} بلاغ</li>
        ))}
      </ul>
    </div>
  );
