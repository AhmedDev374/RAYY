...
def chat_stream(payload: ChatRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # The headers below stop any reverse proxy between the backend and the
    # browser (nginx, Render, Railway) from buffering the response until it is
    # complete, which would hide the stream from the user until the whole
    # answer had already been generated. Harmless when there is no proxy.
