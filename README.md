# رَيّ — RAYY
**رَيّ — RAYY** is a smart irrigation and agricultural water management platform. It combines real-time environmental sensing from ESP32 devices with intelligent, automated irrigation control to optimize water usage and support healthier plant growth.
...
- **Smart irrigation dashboard** — soil moisture, water level, pump status, temperature, humidity, irrigation schedule
- **Water management** — automated and manual irrigation with water-consumption tracking
- **AI disease detection** — trained MobileNetV2 model, 29 classes, ~92.37% accuracy (secondary feature)
- **Smart irrigation assistant** — Gemma 4 powered assistant with live sensor context
...
cp .env.example .env      # configure AUTH_MODE, SUPABASE_URL, DATABASE_URL, GEMINI_API_KEY
...
|----------|-------------|
| `AUTH_MODE` | `supabase` (default) or `legacy`. See [Authentication](#authentication) below. |
| `DATABASE_URL` | PostgreSQL connection string or `sqlite:///./data/app.db` |
| `SUPABASE_URL` | Your Supabase project URL — required when `AUTH_MODE=supabase`, used to fetch Supabase's JWKS to verify access tokens |
| `SECRET_KEY` | Signing key for locally-issued JWTs — only used when `AUTH_MODE=legacy` |
| `GEMINI_API_KEY` | Google AI API key for chat |
| `MODEL_PATH` | Path to `smart_plant_doctor_model.pth` |

> `SUPABASE_JWT_SECRET` is **not** required. Supabase access tokens are asymmetrically signed (ES256/RS256) and verified against Supabase's public JWKS endpoint, not a shared secret.

### Authentication

By default (`AUTH_MODE=supabase`) the backend verifies the `Authorization: Bearer <token>` header sent by the frontend against Supabase's public JWKS (`{SUPABASE_URL}/auth/v1/.well-known/jwks.json`), and creates/looks up a local `User` row keyed on the Supabase `sub` claim. Set `AUTH_MODE=legacy` only if you're using the local email/password auth endpoints instead of Supabase.
