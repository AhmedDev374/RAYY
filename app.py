#!/usr/bin/env python3
"""
رَيّ — RAYY - Streamlit Web Application
Combines smart irrigation sensor monitoring and plant disease detection
"""
...
try:
    ...
    st.error("❌ Could not import the RAYY model. Make sure inference.py is in the ai/ directory.")
    st.stop()
...
def load_model():
    """Load the RAYY disease-detection model"""
    if st.session_state.doctor is None:
        try:
            with st.spinner("💧 Loading RAYY model..."):
...
def home_dashboard():
    ...
    st.markdown('<h1 class="main-header">💧 رَيّ — RAYY | لوحة الري الذكي</h1>', unsafe_allow_html=True)
...
def about_page():
    """About page"""
    st.markdown('<h1 class="main-header">ℹ️ About رَيّ — RAYY</h1>', unsafe_allow_html=True)

    st.markdown("""
    ## 💧 What is رَيّ — RAYY?

    رَيّ — RAYY is an AI-powered smart irrigation and water-management system that combines:
    - **Real-time irrigation sensor monitoring** — soil moisture, temperature, humidity, and light
    - **Intelligent irrigation control** to optimize water usage
    - **Computer vision** for disease detection and diagnosis (secondary feature)
